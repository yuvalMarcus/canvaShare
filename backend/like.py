...
from classes import Like, Likes, LikesNumber
from db_utils import *
from auth import *

router = APIRouter(prefix="/like")

@router.post('', response_model=LikesNumber)
def like(like_obj: Like, jwt_user_id: int = Depends(check_guest_or_blocked)) -> LikesNumber:
...
@router.delete('', response_model=LikesNumber)
def unlike(like_obj: Like, jwt_user_id: int = Depends(check_guest_or_blocked)) -> LikesNumber:
    canvas_id = like_obj.canvas_id
...
def get_canvases_likes_number(canvas_id: Optional[int] = None, user_id: Optional[int] = None) -> Likes:
    results = []
    for canvas in get_canvases_likes(canvas_id, user_id):
        results.append({"canvas_id": canvas[0], "user_id": canvas[1]})
