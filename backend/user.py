...
def update_user(user_id: int, user: User, jwt_user_id: int = Depends(check_guest_or_blocked)) -> dict:
    # Ensure the target user exists
    if not is_user_exist(user_id=user_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    if user_id == jwt_user_id or is_admin(jwt_user_id):
        if user.username:
        ...
        update_user_in_db(user_id=user_id, username=user.username, hashed_password=hashed_password,
                          email=user.email, profile_photo=user.profile_photo, cover_photo=user.cover_photo,
                          about=user.about)
        if user.tags:
            delete_favorite_tags(user_id)
            insert_favorite_tags_to_db(user_id, get_tags_id(user.tags))
        return {}
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorized")
